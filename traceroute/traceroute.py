...
def traceroute(sendsock: util.Socket, recvsock: util.Socket, ip: str) \
    ...
    """
    current_port = TRACEROUTE_PORT_NUMBER
    ...
    for ttl in range(1, TRACEROUTE_MAX_TTL + 1):
        ...
        for _ in range(PROBE_ATTEMPT_COUNT):
            ...
            #Invalid ICMP Type
            if icmp.type != 11 and icmp.type != 3:
                continue
            #Invalid ICMP Code
            ...
            if  ipv4.proto != 1:
                continue
            #Truncated Buffer
            if ipv4.length < ipv4.header_len + 8 + 20 + 8:
                continue

            # Parse the inner IP packet (the one that caused the error)
            icmp_payload_start = ipv4.header_len + 8
            if len(buffer) < icmp_payload_start + 20:
                continue
            #Irrelevant TTL Response
            inner_ip = IPv4(buffer[icmp_payload_start:])
            if inner_ip.dst != ip:
                continue
            # Parse the inner UDP packet
            inner_udp_start = icmp_payload_start + inner_ip.header_len
            if len(buffer) < inner_udp_start + 8:
                continue
            inner_udp = UDP(buffer[inner_udp_start:])
